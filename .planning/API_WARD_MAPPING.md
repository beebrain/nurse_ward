...
- แผนกที่ 1: ชื่อหลัก = A, **ชื่อ API เพิ่มเติม** = B → cron บันทึกแถวแยก A, B | แสดงผลรวม A+B
- แผนกที่ 2: ชื่อหลัก = C

ตั้งค่าใน Admin → จัดการ Ward → แก้ไข → เลือกชื่อหลัก แล้วติ๊กชื่อเพิ่มเติม (ตาราง `ward_api_aliases`)

ข้อมูลชั่วโมงเก็บใน `hourly_patient_census.source_api_ward_name` (แยกแถว) — `HourlyCensusModel` รวมตอนแสดง Handover
